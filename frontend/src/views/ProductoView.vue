    ...
    <div class="product-detail">
      <div class="product-detail__card">
        <div class="product-detail__header">
          <button class="btn" @click="favorito">{{ esFav() ? 'Eliminar de Favoritos' : 'Favoritos'}}</button>
          <div>
            <h1>{{ producto?.nombre || 'Producto' }}</h1>
            <div class="product-detail__seller">
              <i class="bi bi-person"></i>
              <RouterLink v-if="producto?.id_vendedor" :to="`/usuario/${producto.id_vendedor}`">@{{
                producto?.vendedor_nickname || producto?.nickname || 'vendedor' }}</RouterLink>
        ...
        <div class="product-detail__media">
          <img :src="producto?.imagen ? resolveImageSrc(producto.imagen) : '/assets/logo.jpeg'"
            :alt="producto?.nombre || 'Producto'" />
        </div>

        <div class="product-detail__meta">
          <span class="chip">{{ categoriaText }}</span>
          <span class="chip">Stock: {{ formatStock(producto?.stock, producto?.unidad_simbolo || producto?.unidad_nombre)
            }}</span>
        </div>
      ...
      <aside class="product-detail__reserve">
        <template v-if="Number(producto?.stock) > 0">
          ...
          <button class="btn btn-warning btn-sm w-100" type="button" @click="reservar"
            :disabled="!canReserve || reservando">
...
<script setup>
  ...
  async function loadFavoritos() {
    try {
      ...
      productoFav.value = Array.isArray(res.data) ? res.data : [];
    } catch (err) {
  ...
  function esFav(){
    ...
    return productoFav.value.find(a => producto.value.id === a.id);
  }
  ...
  async function favorito() {
    ...
    try {
      ...
      if (!productoFav.value.find(a => String(a.id) === String(producto.value.id))){
        await axios.post(`/favoritos/${producto.value.id}/producto`);
        toast.success('Añadido a favoritos');
      }else{
        await axios.delete(`/favoritos/eliminar/${producto.value.id}`);
  ...
  function esActiva() {
    if (!producto.value) return false;
    return miAlertas.value.find(a => producto.value.id === a.id);
  }
