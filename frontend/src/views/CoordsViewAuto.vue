...
async function autoMyLocation() {
  if (!('geolocation' in navigator)) return;
  if (!map) return;

  const MIN_ACCURACY_METERS = 200;

  await new Promise((resolve) => {
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const latitude = pos && pos.coords ? pos.coords.latitude : null;
        const longitude = pos && pos.coords ? pos.coords.longitude : null;
        const accuracy = pos && pos.coords ? pos.coords.accuracy : null;

        const latOk = Number.isFinite(Number(latitude));
        const lngOk = Number.isFinite(Number(longitude));
        const accOk = Number.isFinite(Number(accuracy));

        if (latOk && lngOk) {
          // Si la precisión es buena, centramos el mapa en tu ubicación.
          // Si la precisión es mala, dejamos el centro por defecto y el usuario puede pinchar manualmente.
          if (!accOk || Number(accuracy) <= MIN_ACCURACY_METERS) {
            map.setView([Number(latitude), Number(longitude)], 14);
          }
        }
