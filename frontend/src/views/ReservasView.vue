      ...
      <div v-else style="display: flex; flex-direction: column; gap: 12px; margin-top: 12px">
        <div v-for="r in activeList" :key="r.id" style="border: 1px solid #e7e7e7; border-radius: 12px; padding: 12px">
          ...
          <div class="actions" style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap">
            ...
            <button v-if="tab === 'pendientes' && isComprador(r)" class="btn btn-primary" type="button"
            ...
            <template v-if="tab === 'aceptadas' && isVendedor(r)">
              <button class="btn" type="button" :disabled="savingById[r.id]" @click="cambiarEstado(r, 'completada')">
...
<script setup>
  ...
  const finalizadas = computed(() => (reservas.value || []).filter((r) => r.estado === 'completada' || r.estado === 'rechazada'));
