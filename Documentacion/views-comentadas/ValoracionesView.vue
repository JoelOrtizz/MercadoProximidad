<!--
VISTA: ValoracionesView (ValoracionesView.vue)

Que pantalla es:
- Esta copia refleja el estado actual de frontend/src/views/ValoracionesView.vue.
...
// esVenta: centraliza una parte concreta de la logica de esta vista documentada.
...
// getDisplayLabels: carga datos y actualiza el estado visible de la vista.
  function getDisplayLabels(item) {
    const soyVendedor = esVenta(item);
    const labelsProducto = ["Producto / Calidad", "Entrega", "Trato"];
    const labelsPersona = ["Seriedad / Pago", "Puntualidad", "ComunicaciÃ³n"];

    if (tab.value === 'enviadas') {
      // SI YO VALORÃ‰:
      // Si soy Vendedor -> ValorÃ© al Comprador (Persona)
      if (soyVendedor) return labelsPersona;
      // Si soy Comprador -> ValorÃ© al Vendedor (Producto)
      return labelsProducto;
    }
    else if (tab.value === 'recibidas') {
      // SI ME VALORARON A MÃ:
  ...
  const formConfig = computed(() => {
    ...
    if (esVenta(estrellas.value)) {
      return {
        ...
        labels: ["Seriedad / Pago", "Puntualidad", "ComunicaciÃ³n"],
      };
    } else {
      return {
...
// abrirValorar: centraliza una parte concreta de la logica de esta vista documentada.
...
// submitValoracion: valida y envia cambios al backend, mostrando resultado al usuario.
  async function submitValoracion() {
    if (!estrellas.value) return;

    if (form.nota_producto === 0 || form.nota_entrega === 0 || form.nota_negociacion === 0) {
      toast.error("Por favor, puntÃºa todos los campos");
      return;
    }
    ...
    try {
      ...
      toast.success("Â¡ValoraciÃ³n enviada!");
...
// loadReservas: carga datos y actualiza el estado visible de la vista.
